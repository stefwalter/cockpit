...
Thus, a 'data' message contains data for one or more point in time
...
For real time monitoring, you will generally only receive one point
intime per 'data' message, but when accessing archived data, the
channel might report multiple points in time in one message, to
improve efficiency.
...
object desribes one metric.  For example:
