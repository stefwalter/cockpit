...
static dbus_bool_t
append_unix_process_subject_to_iter (DBusMessageIter *iter,
                                     dbus_uint32_t pid,
                                     dbus_uint64_t start_time,
                                     dbus_int32_t uid)
{
  const char *unix_process = "unix-process";
  const char *pid_key = "pid";
  const char *start_time_key = "start-time";
  const char *uid_key = "uid";
  DBusMessageIter subj, dict, entry, variant;

  return dbus_message_iter_open_container (&iter, DBUS_TYPE_STRUCT, "sa{sv}", &subj) &&
         dbus_message_iter_append_basic (&subj, DBUS_TYPE_STRING, &unix_process) &&
         dbus_message_iter_open_container (&subj, DBUS_TYPE_ARRAY, "{sv}", &dict) &&

         dbus_message_iter_open_container (&dict, DBUS_TYPE_DICT, "sv", &entry) &&
         dbus_message_iter_append_basic (&entry, DBUS_TYPE_STRING, &pid_key) &&
         dbus_message_iter_open_container (&entry, DBUS_TYPE_VARIANT, "u", &variant) &&
         dbus_message_iter_append_basic (&variant, DBUS_TYPE_UINT32, &pid) &&
         dbus_message_iter_close_container (&entry, &variant) &&
         dbus_message_iter_close_container (&dict, &entry) &&

         dbus_message_iter_open_container (&dict, DBUS_TYPE_DICT, "sv", &entry) &&
         dbus_message_iter_append_basic (&entry, DBUS_TYPE_STRING, &start_time_key) &&
         dbus_message_iter_open_container (&entry, DBUS_TYPE_VARIANT, "t", &variant) &&
         dbus_message_iter_append_basic (&variant, DBUS_TYPE_UINT64, &start_time) &&
         dbus_message_iter_close_container (&entry, &variant) &&
         dbus_message_iter_close_container (&dict, &entry) &&

         dbus_message_iter_open_container (&dict, DBUS_TYPE_DICT, "sv", &entry) &&
         dbus_message_iter_append_basic (&entry, DBUS_TYPE_STRING, &uid_key) &&
         dbus_message_iter_open_container (&entry, DBUS_TYPE_VARIANT, "i", &variant) &&
         dbus_message_iter_append_basic (&variant, DBUS_TYPE_INT32, &uid) &&
         dbus_message_iter_close_container (&entry, &variant) &&
         dbus_message_iter_close_container (&dict, &entry) &&

         dbus_message_iter_close_container (&subj, &dict) &&
         dbus_message_iter_close_container (&iter, &subj);
}

static int
check_authorization (const char *action,
                     pid_t subject_pid,
                     uint64_t subject_start_time,
                     uid_t subject_uid)
{
  ...
  if (!conn)
    {
      message ("couldn't connect to system bus: %s", error.message);
      goto out;
    }

  ret = PAM_BUF_ERR;

  message = dbus_message_new_method_call ("org.freedesktop.PolicyKit1",
                                          "/org/freedesktop/PolicyKit1/Authority",
                                          "org.freedesktop.PolicyKit1.Authority",
                                          "CheckAuthorization");
  if (!message)
    {
      message ("couldn't allocate memory for dbus message");
      goto out;
    }
  ...
  /* (sa{sv}): the subject */
  if (!append_unix_process_subject_to_iter (&iter, subject_pid, subject_start_time, subject_uid) ||
      ...
      !dbus_message_iter_append_string (&iter, DBUS_TYPE_STRING, &cancellation))
    {
      message ("couldn't allocate memory for dbus message");
      goto out;
    }
  ...
  reply = dbus_connection_send_with_reply_and_block (conn, message,
                                                     DBUS_TIMEOUT_INFINITE, &error);
  if (!reply)
    {
      message ("couldn't send message to polkit: %s", error.message);
      goto out;
    }

  sig = dbus_message_get_signature (reply);
  if (!sig || strcmp (sig, "bba{ss}") != 0 ||
      !dbus_message_iter_init (reply, &iter))
    {
      message ("received invalid signature reply from polkit");
      goto out;
    }

  /* first boolean is whether authorized or not */
  authorized = FALSE;
  dbus_message_iter_get_basic (&iter, &authorized);
                     ...
                     const char *argv[])
{
  ...
  parse_args (argc, argv, &action_id);
  ...
  if (!lookup_pid_start_time (pid, &start_time))
    goto out;
  ...
  ret = check_authorization (action_id, pid, start_time, auth_uid);
