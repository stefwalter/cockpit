                        ...
                        gconstpointer data)
{
...
#if 0
  response = cockpit_web_response_new (tc->io, "/cockpit/@localhost/checksum", NULL, NULL);
  cockpit_web_service_resource (tc->service, tc->headers, response);
  ...
  bytes = g_memory_output_stream_steal_as_bytes (tc->output);
  g_printerr ("%.*s", (gint)g_bytes_get_size (bytes), (gchar *)g_bytes_get_data (bytes, NULL));
  g_bytes_unref (bytes);
#endif
