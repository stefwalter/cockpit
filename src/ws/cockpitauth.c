...
static CockpitCreds *
credentials_for_gssapi (const gchar *remote_host,
                        gss_name_t name,
  ...
  if (user)
    {
      creds = cockpit_creds_new (user,
                                 COCKPIT_CRED_RHOST, remote_host,
                                 COCKPIT_CRED_GSSAPI, client,
                                 NULL);
    }
                                  ...
                                  GHashTable *out_headers)
{
  ...
  creds = credentials_for_gssapi (remote_peer, name, client);
  if (creds)
    client = GSS_C_NO_CREDENTIAL;
