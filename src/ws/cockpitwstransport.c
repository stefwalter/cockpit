 ...
 * WebSocket to a cockpit-ws server. Note this is the client side
...
static void
cockpit_ws_transport_send (CockpitTransport *transport,
                           guint channel,
                           GBytes *payload)
{
  ...
  prefix = g_strdup_printf ("%u\n", channel);

  web_socket_connection_send (self, WEB_SOCKET_DATA_TYPE_BINARY,
                              prefix, payload);
...
cockpit_ssh_transport_close (CockpitTransport *transport,
                             const gchar *problem)
{
  CockpitWsTransport *self = COCKPIT_WS_TRANSPORT (transport);
  gboolean closing;
  ...
  if (!closing)
    {
      web_socket_connection_close (self->web_socket,
                                   xxxx_code, problem);
    }
}
...
{
  iface->send = cockpit_ws_transport_send;
  iface->close = cockpit_ws_transport_close;
}
 ...
 */
CockpitTransport *
cockpit_ws_transport_new (const gchar *host,
                          guint port,
                          CockpitCreds *creds)
{
  return g_object_new (COCKPIT_TYPE_WS_TRANSPORT,
                       "host", host,
                       "port", port,
                       "creds", creds,
                       NULL);
 ...
 */
const gchar *
cockpit_ws_transport_get_host_certificate (CockpitWsTransport *self)
{
  g_return_val_if_fail (COCKPIT_IS_WS_TRANSPORT (self), NULL);
  if (!self->data)
    return NULL;
  return self->data->host_certificate;
}

/**
 * cockpit_ws_transport_get_host_fingerprint:
 * @self: the ws tranpsort
 *
 * Get the host fingerprint of the WebSocket connection peer
 * certificate. This is only valid after the transport opens
 ...
 */
const gchar *
cockpit_ws_transport_get_host_fingerprint (CockpitWsTransport *self)
{
  g_return_val_if_fail (COCKPIT_IS_WS_TRANSPORT (self), NULL);
