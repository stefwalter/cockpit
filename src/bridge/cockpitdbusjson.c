                 ...
                 GError *error)
{
  ...
  if (!call->cookie)
    {
      g_debug ("dropping error without cookie: %s", error->message);
      return;
    }
                               ...
                               CallData *call)
{
  ...
  g_debug ("invoking %s %s.%s", call->path, call->interface, call->method);
                     ...
                     gpointer user_data)
{
  ...
  if (val)
    {
      g_debug ("got introspect data for %s", call->path);
                  ...
                  JsonObject *object)
{
  ...
  if (!error && !call->param_type)
    {
      info = g_hash_table_lookup (self->introspect_cache, call->interface);
      if (info)
        {
          g_debug ("found introspect data for %s in cache", call->interface);
          call->param_type = calculate_param_type (info, call->method, &error);
        }
    }
  ...
  if (call->param_type)
    ...
    {
      g_debug ("no introspect data for %s %s", call->path, call->interface);
                   ...
                   gpointer user_data)
{
  if (incoming && g_dbus_message_get_message_type (message) == G_DBUS_MESSAGE_TYPE_SIGNAL)
    process_incoming_signal (user_data, message);
  return message;
}
