...
void
cockpit_pipe_transport_set_interactive (CockpitPipeTransport *self,
                                        gboolean colored,
                                        const gchar *boundary)
{
  g_return_if_fail (COCKPIT_IS_PIPE_TRANSPORT (self));

  self->colored = colored;
  g_free (self->delimiter);
  self->delimiter = boundary ? g_strdup_printf ("\n%s\n", boundary) : NULL;
  self->delimiter_len = self->delimiter ? strlen (self->delimiter) : 0;
}
