  ...
  if (shared)
    {
      ...
      if (shared->claimed == self)
        cockpit_pipe_write (shared->pipe, message);
    }
}
...
cockpit_tty_cleanup (void)
{
  ...
  g_signal_handler_disconnect (shared->pipe, shared->pipe_read);
  g_signal_handler_disconnect (shared->pipe, shared->pipe_close);
  g_object_unref (shared->pipe);
