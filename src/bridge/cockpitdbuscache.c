...
struct _CockpitDBusCache {
  ...
  /* Work around for GDBusInterfaceInfo */
  GList *trash;
  GHashTable *interned;
};
  ...
  for (;;)
    {
      ...
      if (!barrier || (batch && batch->number <= barrier->number))
        return;
                     ...
                     IntrospectData *id)
{
      ...
      if (!iface)
        {
          ...
          /* Make up an empty interface, since the one there didn't work */
  ...
  if (id->callback)
    (id->callback) (iface, id->user_data);
                      ...
                      gpointer user_data)
{
  ...
  g_variant_get (body, "(&s@a{sv}@as)", &interface, &changed, NULL);
  ...
  while (g_variant_iter_loop (&iter, "{o@a{sa{sv}}}", &path, &inner))
    {
      ...
      process_interfaces (self, batch, manager_path, snap,
                          intern_string (self, path), inner);
                             ...
                             GVariant *retval)
{
  ...
  process_paths (self, batch, manager_path, snapshot, inner);
...
static void
barrier_complete (CockpitDBusCache *self,
                  BarrierData *bd)
{
  (bd->callback) (bd->user_data);
  g_slice_free (BarrierData, bd);
}
      ...
      if (!batch)
        {
          for (;;)
            {
              bd = g_queue_pop_head (self->barriers);
              if (!bd)
                break;
              barrier_complete (self, bd);
            }
        }
      else if (g_tree_nnodes (batch) == 0)
        {
          g_queue_pop_head (self->batches);
          for (;;)
            {
              bd = g_queue_pop_head (self->barriers);
              if (!bd || bd->batch != batch)
                break;
              barrier_complete (self, bd);
            }
          g_tree_unref (batch);
                         ...
                         GDBusInterfaceInfo *iface)
{
  ...
  if (interfaces)
    {
      if (!iface)
        return;
      else if (g_hash_table_lookup (interfaces, iface->name))
        return;
    }
  ...
  /* Does it fit our rules */
  if (!cockpit_dbus_rules_match (self->rules, path, iface ? iface->name : NULL, NULL, NULL))
    return;
