...
typedef struct {
  ...
  GHashTable *rules_by_ref;
  GHashTable *rules_by_id;

  gchar *path_prefix;
  gchar *interface_prefix;
  gboolean annotate;
} CockpitDBusJson;
...
static gboolean
check_type (CockpitDBusJson *self,
            JsonNode *node,
            JsonNodeType type,
            GType sub_type,
            GError **error)
...
static GVariant *
parse_json_variant (JsonObject *object,
                    GError **error)
{
...
static GVariant *
parse_json_object_path (CockpitDBusJson *self,
                        JsonNode *node,
                        GError **error)
{
  GVariant *result = NULL;
  ...
  if (str[0] != '/' && str[0] != '\0' && self->path_prefix)
    str = abs = g_strdup_printf ("%s/%s", self->path_prefix, str);
...
static GVariant *
parse_json_signature (CockpitDBusJson *self,
                      JsonNode *node,
                      GError **error)
{
  const gchar *str;
...
static GVariant *
parse_json (CockpitDBusJson *self,
            JsonNode *node,
            const GVariantType *type,
            GError **error)
{
  ...
  if (g_variant_type_is_basic (type))
    {
      if (g_variant_type_equal (type, G_VARIANT_TYPE_BOOLEAN))
        ...
        {
          return parse_json_object_path (self, node, error);
        }
      else if (g_variant_type_equal (type, G_VARIANT_TYPE_SIGNATURE))
        {
          return parse_json_signature (self, node, error);
        }
      else
        {
                        ...
                        GBytes *message)
{
  ...
  if ((node = json_object_get_member (root, "call")) != NULL &&
      JSON_NODE_HOLDS_ARRAY (node))
    {
      handle_dbus_call (root, json_node_get_array (node));
    }
  else if ((node = json_object_get_member (root, "watch")) != NULL)
    {
      handle_dbus_watch (root, xxx);
    }
  else if ((node = json_object_get_member (root, "unwatch")) != NULL)
    {
      handle_dbus_unwatch (root, xxx);
    }
  else
    {
      g_message ("ignoring unsupported dbus command");
    }
...
cockpit_dbus_json_constructed (GObject *object)
{
  ...
  self->name = cockpit_channel_get_option (channel, "service");
  ...
  self->annotate = cockpit_channel_get_bool_option (channel, "annotate");
