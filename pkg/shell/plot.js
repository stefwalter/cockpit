 ...
 */
    ...
    function reset(x_range_seconds, x_stop_seconds) {
        ...
        // XXX - breaks with DST change during x_range.
        var now_date = new Date();
        now = now_date.getTime() - now_date.getTimezoneOffset()*60*1000;

        if (x_stop_seconds !== undefined)
            x_offset = now_date.getTime() - x_stop_seconds*1000;
        else
            x_offset = 0;

        // Fill the plot with about 1000 samples.
        //
        // XXX - do this based on the actual size of the plot.
        //
    ...
    function add_metrics_sum_series(desc, opts) {
        ...
        function fmt_date(t) {
            var d = new Date(t);
            return d.getUTCHours().toString() + ":" + d.getUTCMinutes().toString() + ":" + d.getUTCSeconds().toString();
        }

        function fmt_sample(s) {
            return fmt_date(s[0]);
        }
        ...
        function handle_message(event, message) {
            ...
            if (msg.length) {
                ...
                var now_date = new Date();
                t = (now_date.getTime() - now_date.getTimezoneOffset()*60*1000) + (msg.timestamp - msg.now);
                if (t > timestamp + 2*interval)
        ...
        archive_channel = cockpit.channel({ payload: "metrics1",
                                            source: "/var/log/cockpit/archives",
                                            metrics: metrics,
