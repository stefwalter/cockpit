        ...
        exporters: [
            ...
            function (obj) {
                ...
                function check_con(con) {
                    if (connection_settings(con).type == connection_settings(con).slave_type) {
...
PageNetworkInterface.prototype = {
    ...
    update: function() {
        ...
        } else if (iface && iface.Connections[0]) {
        ...
        function find_main_connection(cons) {
            cons.forEach(function(c) {
                if (connection_settings(self.main_connection).timestamp < connection_settings(c).timestamp)
        ...
        function update_connection_slaves(con) {
            ...
            if (cs.type != "bond" && cs.type != "bridge") {
...
function set_slave(model, master_connection, master_settings, slave_type,
    ...
    function find_main_connection(cons) {
        cons.forEach(function(c) {
            if (connection_settings(main_connection).timestamp < connection_settings(c).timestamp)
                main_connection = c;
        });
    }
    ...
    if (val) {
         ...
         */
        if (cs.master == master_settings.connection.uuid) {
